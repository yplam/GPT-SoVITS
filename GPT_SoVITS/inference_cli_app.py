...
def batch_synthesize(dry_run, model_path, target_csv, output_path):
    ...
    extracted_columns = df.iloc[:, [3, 4, 5, 6]]
    ...
    global_ckpt_dir = 'GPT_weights_v2'
    global_pth_dir = 'SoVITS_weights_v2'
    for name, group in grouped:
        ...
        for file in os.listdir(model_dir):
            if file.endswith('.ckpt'):
                ckpt_file = os.path.join(model_dir, file)
            elif file.endswith('.pth'):
                pth_file = os.path.join(model_dir, file)
        if ckpt_file is None:
            for file in os.listdir(global_ckpt_dir):
                if file.startswith(name) and file.endswith('.ckpt'):
                    ckpt_file = os.path.join(global_ckpt_dir, file)
                    break
        if pth_file is None:
            for file in os.listdir(global_pth_dir):
                if file.startswith(name) and file.endswith('.pth'):
                    pth_file = os.path.join(global_pth_dir, file)
                    break
        ...
        if not dry_run:
            ...
            change_gpt_weights(gpt_path=ckpt_file)
            change_sovits_weights(sovits_path=pth_file)
